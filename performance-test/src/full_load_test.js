...
export default function () {
  ...
  // if the organization has been correctly created => publish
  if (r.status === 201) {
    // sleep(1);
    // Publish the debt position.
    url = `${rootUrl}/organizations/${organization_fiscal_code}`;

    r = http.get(url, params);

    console.log("GetOrganization call - organization_fiscal_code = " + organization_fiscal_code + ", Status = " + r.status);

    check(r, {
      'GetOrganization status is 200': (r) => r.status === 200,
    });

    // if the organization has been correctly visualized => delete
    if (r.status === 200) {
      // sleep(1);
      // Delete organization.
      ...
      check(r, {
        'DeleteOrganization status is 200': (r) => r.status === 200,
      });
    }
    // sleep(2);
  }
